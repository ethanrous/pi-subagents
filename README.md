...
- **FleetView** — Claude Code-style navigable list of `main` + every running subagent rendered below the editor (earliest-launched first). Press `↓` (or `←`) at an empty prompt to jump in, `↑`/`↓` to move the selection, `Enter` to replace pi's main chat with the selected agent's live, auto-updating conversation (looking exactly like the main chat, not an overlay), `Esc` (or selecting `main`) to restore it. While a conversation is shown, anything typed at the prompt goes to that agent instead of the main model. Press `x` (then `x` again to confirm) to stop a running/queued agent. Finished agents linger briefly before dropping out, and a conversation stays open through completion so you can read the final output. Toggle via `/agents → Settings → Fleet view`
- **Conversation view** — select any agent in FleetView or `/agents` to show its full conversation in place of pi's main chat. It runs pi's own interactive-mode rendering directly, against a derived view of the child session, rather than a reimplementation of it — so it is not merely similar to the main chat, it is the same code: the same look, native scrolling, syntax-highlighted Markdown, tool calls paired with their results, and bash executions. `ctrl+o` (pi's own tool-expand toggle) and thinking-block visibility apply to it exactly as they apply to the main chat, because they are the same pi methods, patched to also refresh whichever view is open. Typing at the prompt steers a running agent or resumes a finished one, using the same mechanism `@handle` uses, and `Esc` at an empty prompt returns to the main chat
...
Running [workflows](#subagentworkflow) appear as a single `workflow` row above the agents, carrying their agent counts in place of a description. `Enter` on one opens the same two-pane inspector `/agents → Workflows` does, rather than a conversation view. A run's own agents are *not* listed separately — they belong to the run, which reports for them, so they are filtered out of the fleet list, the above-editor widget, the `/agents` menus and `@handle` resolution exactly as nested children are. They are also outside the `maxConcurrent` pool: the run has its own concurrency cap, and routing a fan-out through the session pool as well would let one workflow starve everything else. The agents are ordered earliest-launched first, and only agents you can actually open are shown (pending/queued agents with no session yet appear once they start). At an **empty prompt**, press `↓` (or `←`) to move focus from the prompt into the list — the keyboard cursor is marked `●`, the rest `○`. The cursor's row renders in the theme's primary text color rather than the muted/dim treatment of the others; an agent with a configured `color` shows its badge there too, bolded. `↑`/`↓` move the cursor, `Enter` replaces pi's main chat area with the selected agent's live conversation — it runs pi's own rendering directly against the child session (see [Conversation view](#features)), so it auto-updates as the agent works and looks and scrolls exactly like the main chat because it *is* the main chat's own code — and `Esc` (or `↑` above `main`) returns to the prompt without disturbing an open conversation.

Selecting `main`, or pressing `Esc` at an empty prompt while a conversation is shown, restores pi's own transcript. The list stays live and navigable the whole time a conversation is shown, since it is not modal, so `↓`/`Enter` switches straight from one agent's conversation to another's. Once you leave the list (typing, or `↑` past `main`), the keyboard-cursor `●` goes away entirely and the row actually shown in the main chat area — `main` by default — carries a hollow `◉` instead, so it's never ambiguous which conversation you're looking at versus where the cursor last was. Press `x` (then `x` again to confirm) on a running or queued row to stop it. While a conversation is shown, whatever you type at the prompt and submit goes to *that* agent instead of the main model, steering it if it's still running or queued, resuming it if it finished, using the same mechanism `@handle` uses; typing an explicit `@handle` or `@main` always overrides this and goes where it says. `ctrl+o` (pi's own tool-expand toggle) and thinking-block visibility apply to a shown conversation exactly as they apply to the main chat. A conversation stays open when its agent finishes so you can read the final output, and finished agents linger in the list for a few seconds before dropping out. Typing anything at a non-empty prompt behaves normally — the list only captures arrow keys when the prompt is empty. Disable it entirely via `/agents → Settings → Fleet view`.
...
- **Running agents** — select one to show its live conversation in pi's main chat area (same as opening it from FleetView). While it's shown, whatever you type at the prompt steers the agent if it's still running or queued, or resumes it if it finished; `Esc` at an empty prompt returns to the main chat. Stop/abort a still-running agent from [FleetView](#fleetview) with `x` (then `x` again to confirm), including **background** agents, which a global Esc can't unambiguously target (Esc still stops a blocking foreground `Agent` call). A stopped agent reports its partial output flagged as incomplete, not as a completion. `ctrl+o` expands/collapses every tool's output; thinking-block visibility follows pi's own toggle, and both apply to a shown conversation exactly as they apply to the main chat.
...
Runtime tuning values set via `/agents` → Settings (max concurrency, max foreground concurrency, default max turns, grace turns, nested depth, fallback agent, default join mode, scheduling on/off, scope models on/off, disable defaults on/off, strict agent files on/off, agent mentions on/off, output transcript on/off, tool description full/compact/custom, widget all/background/off, usage reporting on/off, cost display on/off, model display on/off) persist across pi restarts. Two files, merged on load:
...
**Show cost** (`showCost`, default `false`): whether the subagent surfaces print an estimated cost beside their token counts — the widget (running *and* finished lines), [FleetView](#fleetview), the conversation view, foreground results, `get_subagent_result`, and completion notifications:
...
src/
  ...
  ui/
    ...
    fleet-list.ts         # FleetView: navigable agent list below the editor; owns the conversation view lifecycle
    agent-view.ts         # Captures pi's live InteractiveMode and runs its own rendering against a child session
